...
const createBookingList = () => {
    $("#BookingTable tbody").empty(); //Removes all children, if the exist.

    fetch("http://localhost:8080/Workshop_7_war_exploded/api/booking", {
        method: 'GET'
        ...
        for(let i=0; i<data.length; i++) {
            ...
            editAnchor.setAttribute('href', `http://localhost:8080/Workshop_7_war_exploded/bookingdetails/EditBooking/${currentItem[currentKeys[0]]}`);
            //Adds the URI needed for the delete method to work.
            btnDel.setAttribute('data-uri', `http://localhost:8080/Workshop_7_war_exploded/api/booking/${currentItem[currentKeys[0]]}`);
